...
typedef void (*smartCmdCB_t)(StomaSense::Stream*, const SmartCmdArguments*, const char*);
typedef void (*serialDefaultCmdCB_t)(StomaSense::Stream*, const char*);

void __defaultCommandNotRecognizedCB(StomaSense::Stream *stream, const char *cmd);
...
public:
    ...
    virtual void callback(StomaSense::Stream *stream, const SmartCmdArguments *args) const = 0;
};
...
public:
    ...
    void callback(StomaSense::Stream *stream, const SmartCmdArguments *args) const;
};
...
{
private:
StomaSense::Stream *const _stream = nullptr;
...
public:
    constexpr SmartComm(const SmartCmdBase *const cmds[N_CMDS], StomaSense::Stream &stream, char endChar = '\n', char sepChar = ' ', serialDefaultCmdCB_t defaultCB=__defaultCommandNotRecognizedCB);
    void tick();
};

template<_smart_comm_size_t N_CMDS>
constexpr SmartComm<N_CMDS>::SmartComm(const SmartCmdBase *const cmds[N_CMDS], StomaSense::Stream &stream, char endChar, char sepChar, serialDefaultCmdCB_t defaultCB)
